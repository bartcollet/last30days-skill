...
Fork-native adaptation of the upstream module: uses this fork's http helper and
a local tty-gated logger; everything else (query expansion, the common-word
false-positive guards _NOISE_WORDS / _passes_topic_filter / LOW_SIGNAL_QUERY_TOKENS,
and the _MIN_RELEVANCE drop-all floor) is ported verbatim. Relevance is the
shared heuristic token_overlap_relevance (no LLM call, so Polymarket stays
cost-free).
"""
...
def filter_items_against_topic(topic: str, items: List[Any]) -> List[Any]:
    ...
    Uses the looser _passes_any_informative_word rule (ANY entity name match is
    sufficient). Accepts a list of either raw dicts (with 'title') or
    SourceItem-like objects (with .title attribute). Returns the filtered list.
    """
    if not topic:
        return items
...
def filter_items_against_keywords(items: List[Any], keywords: List[str]) -> List[Any]:
    ...
    via explicit keywords (e.g., 'nba,gsw,golden-state').
    """
    if not keywords:
        return items
...
def _extract_domain_queries(topic: str, events: List[Dict]) -> List[str]:
    ...
    domain categories. Falls back to frequent title bigrams if no useful tags exist.
    """
