"""HTML rendering for shareable last30days briefs.

Fork-native port of the upstream `--emit=html` feature (mvanhorn `b1773be`,
2026-05-02). Takes a synthesis markdown string and produces a self-contained
HTML document with inline CSS, dark/light mode, and a print stylesheet.
No JavaScript, no external assets beyond Google Fonts (which fall back to
system fonts offline).

Differences vs upstream:
- No `schema.Report` dependency. Caller passes (markdown, topic, date).
- No engine-footer protect/wrap or META-marker promotion (upstream-specific).
- No PROSE_LABELS promotion or evidence-block stripping (upstream-specific).
- Simplified colophon.
"""
...
<body>
<div class="badge"><span class="accent">🌐</span> last30days · __TOPIC__</div>
__BODY__
__COLOPHON__
</body>
</html>
...
def render_html(
    ...
    *,
    ...
    """Render a synthesis markdown string as a self-contained HTML brief.

    Args:
        synthesis_md: the synthesis content as markdown.
        topic: the research topic, shown in <title> and badge.
        generated_date: ISO date string (YYYY-MM-DD); defaults to today.

    Returns:
        A complete HTML5 document as a string.
    """
    body = _markdown_to_html(synthesis_md.strip())
    colophon = _build_colophon(topic=topic, generated_date=generated_date)
...
def _build_colophon(*, topic: str, generated_date: str | None = None) -> str:
    if generated_date is None:
        generated_date = _date_cls.today().isoformat()
    escaped_topic = html.escape(topic)
    rerun_cmd = html.escape(f'/last30days {topic}')
    return (
        '<div class="colophon">'
        f'Generated {generated_date} by last30days. '
        f'Topic: {escaped_topic}. '
        f'Rerun: <span class="rerun">{rerun_cmd}</span>'
        '</div>'
    )


def _wrap_in_template(body: str, colophon: str, topic: str) -> str:
    title = html.escape(topic)
    return (
        HTML_TEMPLATE
        .replace("__TITLE__", title)
        .replace("__TOPIC__", title)
        .replace("__CSS__", CSS)
