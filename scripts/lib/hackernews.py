...
No API key needed - just HTTP calls via the shared http helper.

Fork-native adaptation of the upstream module: uses this fork's http helper
and a local tty-gated logger, reuses openai_reddit._extract_core_subject for
query cleanup, and scores relevance via the shared heuristic
relevance.token_overlap_relevance (no LLM call, so HN stays cost-free).
"""
...
def _extract_core_subject(topic: str) -> str:
    """Clean the raw topic into a core subject for Algolia matching.
...
def _title_matches_query(title: str, query: str, author: str = "") -> bool:
    ...
    Token-overlap relevance scoring at parse time demotes hits where only one
    weak token matched, so the loosened any-word gate won't surface noise to
    the top of the ranking. Word-boundary matching (rather than naive
    substring) prevents short tokens like ``ai`` or ``ts`` from matching
    unrelated words like ``email`` or ``artists``.
    """
    if not query:
        return True
